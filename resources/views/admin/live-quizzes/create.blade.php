...
<div class="container">
    ...
    <form action="{{ route('admin.live-quizzes.store') }}" method="POST">
        @csrf
        <div class="mb-3">
            ...
            <select name="live_session_id" id="live_session_id" class="form-control @error('live_session_id') is-invalid @enderror" required>
                ...
                @foreach($liveSessions as $session)
                    <option value="{{ $session->id }}" {{ old('live_session_id') == $session->id ? 'selected' : '' }}>
                        {{ $session->title }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="d-flex justify-content-between">
            ...
            <button type="submit" class="btn btn-success">Create Live Quiz</button>
        </div>
    </form>
</div>
@endsection 